...
- [x] **Avatar upload — ✅ DONE (code + console).** Now on **Blaze**, so the old Spark
      blocker is gone. Implemented end-to-end in code: a `photoUrl` field on
      `UserProfile`/`users/{uid}`; **Edit profile** has a tappable avatar (Android Photo
      Picker → downscale to 512px JPEG via `ImageUtils` → upload to Cloud Storage
      `avatars/{uid}.jpg` → persist the download URL), with an upload spinner + camera
      badge; a reusable `ProfileAvatar` renders the photo (Coil) with an initials
      fallback on Profile + Edit profile. `storage.rules` rewritten from deny-all to
      owner-scoped avatar writes (≤5MB, image/* only) with signed-in read;
      `firestore.rules` `users update` gains a `photoUrl`-only branch.
      **Console enablement done:** Storage is provisioned on `echo-2b5ba` and the
      avatar-scoped `storage.rules` are **deployed live** (verified 2026-06-21 — the
      project's live Storage rules are the owner-scoped avatar rules, not deny-all);
      the `firestore.rules` `photoUrl` branch is deployed.
      **Posts/comments now show avatars too** *(2026-06-20, commits `d60ae67` +
      `7be64ec`).* The previously-deferred work landed: a denormalized photo plus an
      `AuthorAvatarResolver` that resolves each author's *current* avatar live by
      `authorId`, so a photo change reflects on their older posts/comments as well.
